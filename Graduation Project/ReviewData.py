...
while plotingIndex<len(rows) and plotingIndex>=0:
    ...
    if(ploter.isLookingFor(anamoly.anamolyType) and wantToReview(reviewLabledData,rows[plotingIndex]['value'] )):
        ...
        sampled = preprossing(anamoly , shifting=True , smoothing=True , padding=True)
        # anamolyArray.append((sampled , 'Oiginal anomaly after preprocessing')) ;
        dis = getPureDisplacement(anamoly,50  )
        anamolyArray.append((dis, 'displacement without detrending'))

        #################### geting anaomaly features ####################################
